...
int main(int argc, char** argv) {
    ...
    // Appliquer le filtre blur sur la première moitié de l'image d'entrée avec le premier stream
    boxBlurFilter<<<gridSize, blockSize, blockSize.x * blockSize.y * sizeof(unsigned char), stream1>>>(d_input, d_output, width, (height / 2) + radius, radius);
    ...
    // Appliquer le filtre blur sur la deuxième moitié de l'image d'entrée avec le deuxième stream
    boxBlurFilter<<<gridSize, blockSize, blockSize.x * blockSize.y * sizeof(unsigned char), stream2>>>(d_input + width * ((height / 2) - radius), d_output + width * ((height / 2) - radius), width, (height / 2) + radius, radius);
    kernelStatus = cudaGetLastError();
    if (kernelStatus != cudaSuccess) {
        std::cout << "CUDA Error: " << cudaGetErrorString(kernelStatus) << std::endl;
        exit(-1);
    ...
    file << "blur_shared_stream : " << duration << " ms\n" << std::endl;
    file.close();
