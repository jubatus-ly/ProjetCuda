...
int main(int argc, char** argv) {
    ...
    sobelFilter<<<gridSize, blockSize, 0, stream1>>>(d_input, d_output, width, height / 2);
    ...
    sobelFilter<<<gridSize, blockSize, 0, stream2>>>(d_input + width * (height / 2), d_output + width * (height / 2), width, height / 2);
    kernelStatus = cudaGetLastError();
    if (kernelStatus != cudaSuccess) {
        std::cout << "CUDA Error: " << cudaGetErrorString(kernelStatus) << std::endl;
        exit(-1);
